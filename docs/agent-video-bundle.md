...
한 가지 예상해 둘 값: 회수된 `project.duration` 은 클립 끝 최대값이 아니라 **최소 10초로 바닥이 잡힌다**. 앱의 로드 경로가 `Math.max(클립끝, 10)` 으로 타임라인 길이를 정하고(`useVideoProjectLibrary`), 그 값이 autosave 에 기록되기 때문이다. 6초짜리 번들을 넣고 그대로 회수하면 `duration: 10` 으로 나온다 — 브리지가 고친 것이 아니라 에디터의 기존 동작이고, 두 번째 왕복부터는 값이 더 움직이지 않는다. 클립 타이밍은 영향받지 않는다.
