...
import { getPanelContent, getPanelTitle, getPanelDefaultSize } from "./PanelRegistry";
...
function FloatingWindowComponent({
  ...
  const { startDragging, endDragging, updateDropTarget, layoutState, dockWindow } = useLayout();

  const [isDragging, setIsDragging] = useState(false);
  const [isResizing, setIsResizing] = useState(false);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });

  const title = getPanelTitle(window.panelId);
  const content = getPanelContent(window.panelId);
  const defaultSize = getPanelDefaultSize(window.panelId);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging) {
        ...
        // Dock zone detection is now handled by individual Panel components
      }
